...
@section('content')
    <div class="max-w-2xl">
        <div class="mb-6">
            <p class="text-sm text-slate-500">Update category information</p>
            ...
            <form action="{{ route('admin.categories.update', $category) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <textarea name="dec" id="dec" rows="4" required
                            class="w-full rounded-lg border-slate-300 border px-4 py-2 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 @error('dec') border-red-500 @enderror">{{ old('dec', $category->dec) }}</textarea>
                        @error('dec')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input type="file" name="image" id="image" accept="image/*"
                            class="w-full rounded-lg border-slate-300 border px-4 py-2 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 @error('image') border-red-500 @enderror">
                        @error('image')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                        ...
                        @if ($category->image)
                            <div class="mt-3">
                                <p class="text-xs font-semibold text-slate-600 mb-1">Current image</p>
                ...
                <div class="mt-8 flex justify-end space-x-4">
                    <a href="{{ route('admin.categories.index') }}"
                        class="px-4 py-2 border border-slate-300 rounded-lg text-slate-700 hover:bg-slate-50">Cancel</a>
