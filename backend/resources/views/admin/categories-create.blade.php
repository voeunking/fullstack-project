...
@section('content')
    <div class="mb-8">
        <div
            class="overflow-hidden rounded-[2rem] border border-white/10 bg-gradient-to-br from-slate-950 to-cyan-900/20 shadow-2xl shadow-cyan-900/10">
        ...
        <div class="rounded-[2rem] border border-slate-200/70 bg-white/90 p-6 shadow-xl shadow-slate-200/70">
            <form action="{{ route('admin.categories.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        <label for="image" class="block text-sm font-bold text-slate-700 mb-1">Category Image</label>
                        <input type="file" name="image" id="image" accept="image/*"
                            class="w-full rounded-2xl border-slate-300 border px-4 py-3 focus:ring-2 focus:ring-cyan-500/20 focus:border-cyan-500 @error('image') border-red-500 @enderror">
                        @error('image')
                            <p class="mt-2 text-sm font-bold text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div class="mt-8 flex flex-col sm:flex-row justify-end gap-3">
                    <a href="{{ route('admin.categories.index') }}"
                        class="px-5 py-3 text-sm font-bold rounded-2xl border border-slate-200 text-slate-700 hover:bg-slate-50">Cancel</a>
                    <button type="submit"
                        class="px-5 py-3 text-sm font-bold rounded-2xl bg-cyan-400 text-slate-950 shadow-lg shadow-cyan-400/20 hover:bg-cyan-300">
                        Create Category
                    </button>
                </div>
            </form>
        ...
        <aside class="rounded-[2rem] border border-slate-200/70 bg-slate-50/70 p-6 shadow-xl shadow-slate-200/70">
            ...
            <div class="mt-4 rounded-[1.5rem] border border-slate-200 bg-white p-4">
                ...
                <p class="mt-2 text-sm font-semibold text-slate-600">
                    {{ \Illuminate\Support\Str::limit(old('description', 'Write a short description...'), 120) }}</p>
                <div class="mt-4 flex items-center justify-between">
                    <span
                        class="inline-flex items-center rounded-xl bg-slate-100 px-3 py-1 text-xs font-bold text-slate-600">Ready</span>
